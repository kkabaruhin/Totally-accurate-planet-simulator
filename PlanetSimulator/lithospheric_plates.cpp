...
Bitmap^ geomap::create_bitmap_of_height() {
	...
	double my_max_altitude = max_altitude;
	if (abs(max_altitude) < 1)
	{
		my_max_altitude = 1;
	}
	...
	for (int i = 1; i < width; i++) {
		for (int j = 1; j < height; j++) {
			double multiplier = (abs(altitude_map[i][j] - min_altitude) / (my_max_altitude - min_altitude));
			int curr_red = (int)(left_red + (right_red - left_red) * multiplier);
			int curr_green = (int)(left_green + (right_green - left_green) * multiplier);
			int curr_blue = (int)(left_blue + (right_blue - left_blue) * multiplier);
